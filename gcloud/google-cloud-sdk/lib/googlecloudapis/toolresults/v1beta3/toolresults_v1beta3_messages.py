...
class ToolExecution(messages.Message):
  ...
  Fields:
    ...
    deprecatedToolOutputs: References to opaque files of any format output by
      the tool execution.  The maximum allowed number of tool outputs per step
      is 1000.  Deprecated. This field will be replaced with a new
      tool_outputs field, but in a different type.  - In response: present if
  ...
  deprecatedToolOutputs = messages.MessageField('FileReference', 2, repeated=True)
  exitCode = messages.MessageField('ToolExitCode', 3)
  toolLogs = messages.MessageField('FileReference', 4, repeated=True)
  toolOutputs = messages.MessageField('ToolOutputReference', 5, repeated=True)
